...
// Cordis injection contract: wait for the `skills` service (provided by
// `@deepseek-ai/dsh-skill`, mounted by the host base bundle) before apply().
// The loader starts all plugin entries in parallel, so without `inject` the
// apply() can run before the skills service fiber is active — then
// ctx.get('skills') returns undefined (strict) and never retries. Same
// contract as @deepseek-ai/dsh-skill-filesystem and @deepseek-ai/dsh-skill-badge.
